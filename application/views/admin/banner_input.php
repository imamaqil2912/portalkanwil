...
<form method="post" action="<?php echo base_url(); ?>index.php/admin/insert_banner" enctype="multipart/form-data">
...
<table width="100%" border="1" cellspacing="0">
	...
	echo '<tr><td width="120" style="padding:5px;">'.$tp['kode_banner'].'</td><td style="padding:5px;">'.strip_tags($tp['judul']).'</td><td style="padding:5px;">'.strip_tags(substr($tp['deskripsi'],0,100)).'...</td><td style="padding:5px;">'.$status.'</td><td style="padding:5px;"><a href="'.base_url().'index.php/admin/edit_banner/'.$tp['kode_banner'].'"><img src="'.base_url().'assets/images/edit-icon.gif" border=0> Edit</a></td><td style="padding:5px;"><a href="'.base_url().'index.php/admin/hapus_banner/'.$tp['kode_banner'].'/'.$tp['gambar'].'" onClick=\'return confirm("Anda yakin ingin menghapus konten ini???")\'><img src="'.base_url().'assets/images/hapus.png" border=0> Hapus</a></td></tr>';
	}
	?>
</table>
