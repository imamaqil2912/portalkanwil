...
<title></title>
<link rel="stylesheet" href="<?php echo base_url(); ?>assets/css/screen_admin.css" type="text/css" media="screen" title="default" />
...
<br />
<!--inputan jquery-->
		<fieldset style="border:1px dashed #666666; width:98%; padding:10px; display:none;" id="menukanan"> <!--memberi garis tepi putus''--> 
<legend align="left"><strong>Form Tambah Slider :</strong></legend>
<form method="post" action="<?php echo base_url(); ?>slider/insert_slider" enctype="multipart/form-data">
...
<legend align="left"><strong>Tampilkan Semua Slider :</strong></legend>
<table width="100%" border="1" cellspacing="0">
	...
	echo '<tr><td width="120" style="padding:5px;">'.$tp['kode_banner'].'</td><td style="padding:5px;">'.strip_tags($tp['judul']).'</td><td style="padding:5px;">'.strip_tags(substr($tp['deskripsi'],0,100)).'...</td><td style="padding:5px;">'.$status.'</td><td style="padding:5px;"><a href="'.base_url().'slider/edit_slider/'.$tp['kode_banner'].'"><img src="'.base_url().'assets/images/edit-icon.gif" border=0> Edit</a></td><td style="padding:5px;"><a href="'.base_url().'slider/hapus_slider/'.$tp['kode_banner'].'/'.$tp['gambar'].'" onClick=\'return confirm("Anda yakin ingin menghapus konten ini???")\'><img src="'.base_url().'assets/images/hapus.png" border=0> Hapus</a></td></tr>';
	}
	?>
</table>
